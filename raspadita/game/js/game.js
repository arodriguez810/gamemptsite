  ...
  switch (page) {
    ...
    case 'result':
      ...
      TweenMax.to(tweenData, .5, {
        tweenScore: playerData.credit, overwrite: true, onUpdate: function () {
...
function animateBlink(obj, alpha) {
  ...
  obj.alpha = alphaNum;
  TweenMax.to(obj, .3, {
    alpha: .5, overwrite: true, onComplete: function () {
      TweenMax.to(obj, .3, {
        alpha: alphaNum,
        overwrite: true,
        onComplete: animateBlink,
        onCompleteParams: [obj, alpha]
      });
    }
  });
}
...
function animatePrize(obj, alpha) {
  ...
  obj.alpha = alphaNum;
  TweenMax.to(obj, .3, {
    alpha: .5, scaleX: scaleNum, scaleY: scaleNum, overwrite: true, onComplete: function () {
      TweenMax.to(obj, .3, {
        alpha: alphaNum,
        scaleX: 1,
        scaleY: 1,
...
function animateCredit() {
  var scaleNum = 1.5;
  TweenMax.to(gameScoreTxt, .2, {
    scaleX: scaleNum, scaleY: scaleNum, overwrite: true, onComplete: function () {
      TweenMax.to(gameScoreTxt, .2, {scaleX: 1, scaleY: 1, overwrite: true});
    }
  });

  TweenMax.to(gameScoreRedTxt, .2, {
    alpha: 1, scaleX: scaleNum, scaleY: scaleNum, overwrite: true, onComplete: function () {
...
async function buyCard() {
  ...
  await useCredit(playerData.credit);
  updateGameScore();
  showWonStatus();
  showCardButtons();
  toggleCoinsFall(false);
...
function proceedValidatCard(result) {
  ...
  TweenMax.to(cardMoveContainer, .3, {
    x: 0,
    y: 0,
    alpha: 1,
    rotation: 0,
...
function showScratchComplete() {
  ...
  if (totalWin > 0) {
    ...
    useCredit(playerData.credit).then(d => {
      updateGameScore();
...
function revealPrizes() {
  ...
  for (var n = 0; n < cardsSettings[gameData.cardIndex].items.length; n++) {
    ...
    var delayNum = n * .1;
    TweenMax.to(scratchShape, tweenSpeed, {
      delay: delayNum,
      bezier: {type: "thru", values: strokePosition, curviness: gameSettings.revealCurviness, autoRotate: false},
      ease: Linear.easeNone,
      overwrite: true,
      onUpdate: revealUpdate,
      onUpdateParams: [n],
      onComplete: revealPrizesComplete
    });
  }
  showCardButtons();
}
